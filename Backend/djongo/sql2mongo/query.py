...
class InsertQuery(DMLQuery):
    ...
    def _table(self, statement: SQLStatement):
        tok = statement.next()
        while tok.is_whitespace:
            tok = statement.next()

        try:
            collection = tok.get_name()
        except AttributeError:
             collection = tok.value.strip('"').strip('`')
    ...
    def _fill_values(self, statement: SQLStatement):
        for tok in statement:
            ...
            # Handle aggregated VALUES (...) token from sqlparse 0.4.4
            if not isinstance(tok, Parenthesis) and tok.value.upper().startswith('VALUES') and '(' in tok.value:
                 # Extract the parenthesis part directly
                 idx = tok.value.find('(')
                 if idx != -1:
                     param_str = tok.value[idx:]
                     parsed = sqlparse(param_str)[0]
                     # The first token of the parsed statement should be Parenthesis
                     if parsed.tokens and isinstance(parsed.tokens[0], Parenthesis):
                         tok = parsed.tokens[0]
            ...
            elif not (tok.match(tokens.Keyword, 'VALUES') or tok.value.upper() == 'VALUES'):
                # Debugging info if needed
                print(f"DEBUG Invalid Token: {tok} Type: {tok.ttype} Value: {tok.value}") 
                raise SQLDecodeError
    ...
    def parse(self):
        ...
        # Robustly find table name (after INTO)
        found = False
        for _ in range(10): 
            tok = statement.next()
            if tok.match(tokens.Keyword, 'INTO') or tok.value.upper() == 'INSERT INTO':
                found = True
                break
        
        if not found:
            # Fallback or error? defaulting to trying next token
            pass
